...
import Formgateposition from '../../../SubPages/basic-data/GatePosition/Formgateposition.js'
import {postGatePosition, addGatePosition, modifyGatePosition, removeGatePosition, postDamTree, postRegionTree} from '../../../../../data/dataStore.js';
...
// import "./style.css";
class GatePosition extends BaseSubPage {
    ...
    render() {
        const columns = [
            ...
            {title: '防护等级', dataIndex: 'protectionLevel', width: "8%"},
            {title: '转速', dataIndex: 'rotationRate', width: "8%"},
            {title: '最大圈数', dataIndex: 'maxCircleNum', width: "8%"},
            {title: '最高工作水温', dataIndex: 'maxWorkAmbientTemperature', width: "8%"},
            {title: '最低工作水温', dataIndex: 'minWorkAmbientTemperature', width: "8%"},
    ...
    async loadData() {
        let listPromise = postGatePosition({}).then((res) =>{ return res.ok ? res.json() : Promise.reject("接口出错");}).then((data) => {
            if (data.code === 200) {
                return data.data;
            }
    ...
    createModal() {
        let modal = 
         <Modal visible={this.state.showModal} width={"90%"} 
            ...
            >
            <Formgateposition ref={(node) => { this.f = node ;}} disabled={false}/>
    ...
    modifyClick(text, record, index) {
        ...
        this.setState({
            ...
            form.setFieldsValue({
                gpName : record.name,
                gpCode: record.serialNumber,
                gpModel: record.model,
                gpSpec: record.spec,
                gpRange: record.protectionLevel,
                gpRotationRate: record.rotationRate,
                gpMaxCircleNum : record.maxCircleNum,
                gpMaxWorkWaterTemperature: record.maxWorkAmbientTemperature,
                gpMinWorkWaterTemperature: record.minWorkAmbientTemperature,
                gpManufacturer: record.manufacturerId,
                gpChineseShort: record.chineseShort,
                gpPinyinInitial: record.pinyinInitial
            });
    ...
    add() {
        ...
        form.validateFields((err)=> {
            if (!err) {
                ...
                let data =  {
                    name : item.gpName,
                    serialNumber: item.gpCode,
                    model: item.gpModel,
                    spec: item.gpSpec,
                    protectionLevel: item.gpRange,
                    rotationRate: item.gpRotationRate,
                    maxCircleNum : item.gpMaxCircleNum,
                    maxWorkAmbientTemperature: item.gpMaxWorkWaterTemperature,
                    minWorkAmbientTemperature: item.gpMinWorkWaterTemperature,
                    manufacturerId: item.gpManufacturer,
                    chineseShort: item.gpChineseShort,
                    pinyinInitial: item.gpPinyinInitial
                };
                addGatePosition(data).then((res) => {
                    if (res.ok) {
                        return res.json();
                    }
    ...
    modify(id) {
        ...
        form.validateFields((err)=> {
            if (!err) {
                ...
                modifyGatePosition(data).then((res) => {
                    if (res.ok) {
                        return res.json();
                    }
                    return Promise.reject("接口出错");
    ...
    remove(record) {
        ...
        removeGatePosition(data).then((res) => {
            if (res.ok) {
                return res.json();
            }
            return Promise.reject("接口出错");
