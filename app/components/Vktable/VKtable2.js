...
class Vktable extends React.Component {
    constructor(props, context) {
        ...
        this.columns = this.enableColumns(props.columns);
    }

    render() {
        return (
            <div>
                <Table {...this.props} loading={this.state.loading} columns={this.columns} dataSource={this.state.dataSource}/>
            </div>
        );
    }
        ...
        columns.forEach((item) => {
            item.render = (text, record, index) => {
                ...
                if (this.state.enable) {
                    switch(item.type){
                        case "Input": ele = <Input style={{ width: item.componentsWidth?item.componentsWidth:'100%' }} defaultValue={text} onChange={this.itemChange.bind(this, record, item, index)}/>; break;
                        case "Number": ele = <InputNumber style={{ width: item.componentsWidth?item.componentsWidth:'100%' }} defaultValue={text} onChange={this.itemChange.bind(this, record, item, index)}/>; break;
                        case "Select": 
                            let data = item.getData && item.getData();
                            let options = data && data.map((d) => { return <Option key={d.value} value={d.value}>{d.text}</Option>});
                            ele = <Select showSearch filterOption={this.filterOption} style={{ width: item.componentsWidth?item.componentsWidth:'100%' }} defaultValue={text} onChange={this.itemChange.bind(this, record, item, index)}>{options}</Select>; break;
                        // case "DatePicker":
                        // case "TreeSelect":
                        default: break;
                    }
    ...
    filterOption(value, option) {
        if (React.isValidElement(option)) return;
